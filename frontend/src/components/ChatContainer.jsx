...
const ChatContainer = () => {
  ...
  // User selected
  return (
    <div className="flex-1 flex flex-col bg-white border-l border-slate-200" style={{ height: '90vh', maxHeight: '90vh' }}>
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-200 bg-white shrink-0">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1.5">
            <span className={`w-1.5 h-1.5 rounded-full ${onlineUsers.includes(selectedUser._id) ? "bg-emerald-500" : "bg-slate-300"}`} />
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-2.5 bg-slate-50">
          ...
          messages.map((each) => (
            <div key={each._id}
              className={
                each.senderId === authUser._id
                  ? "flex items-end gap-2 max-w-xs self-end"
            ...
            >
              ...
              <div className='gap-2 flex-col flex'>
                ...
                <div className={`text-xs text-slate-400 mt-1 ${each.senderId === authUser._id ? "text-right" : "text-left"}`}>
      ...
      <div className="px-5 py-3 border-t border-slate-200 bg-white shrink-0">
        ...
        {selectedFile && (
          <div className="flex items-center gap-2 mb-2 px-1">
            <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-lg px-3 py-1.5 text-xs text-indigo-700 font-medium max-w-xs truncate">
        ...
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-50 transition-all">

          {/* File button */}
          <label className="cursor-pointer text-slate-400 hover:text-indigo-500 transition-colors shrink-0">
            <input
              type="file"
              className="hidden"
          ...
          {/* Send button */}
          <button onClick={handleSendMessage} className="w-7 h-7 bg-indigo-600 hover:bg-indigo-700 rounded-lg flex items-center justify-center transition-colors shrink-0">
